...
import { Class, $, $$, PElement, PObserveEvent, PStopObserving, PFireEvent, getEventMatchingParentElement } from '../shims/prototypeShim';
...
var NodetypeSelectionBubble = Class.create({
  ...
  initialize : function(siblingMode) {
    ...
    this.element = PElement('div', {'class' : 'callout'});
    this.element._p_insert(PElement('span', {'class' : 'callout-handle'}));

    var container = PElement('div', {'class' : 'node-type-options'});
    this.expandedOptionsContainer = PElement('div', {'class' : 'node-type-options-extended'});
    this.element._p_insert(container);
    this.element._p_insert(this.expandedOptionsContainer);

    var _this = this;
    this.buttonsDefs.forEach(function(def) {
      if (!siblingMode || (def.hasOwnProperty('inSiblingMode') && def.inSiblingMode)) {
        container._p_insert(def.type == 'separator' ? _this._generateSeparator() : _this._createOption(def));
      }
    });
    this.element._p_hide();
    editor.getWorkspace().getWorkArea()._p_insert(this.element);
  ...
  _createOption : function(data) {
    ...
    var o = PElement('a', {
      ...
      'title' : data.tip,
      'href' : '#'
    })._p_update(data.symbol); // TODO: eliminate symbol, do "._p_update(data.label)", add style (icons);
    var _this = this;
    o._p_observe('click', function(event) {
    ...
    var container = PElement('span');
    container._p_insert(o);
    expandablePrefix && container._p_insert(this.generateExpandArrow(data));
    return container;
  },
  ...
  handleCreateAction: function(data) {
    ...
    if (nodeType == 'Person') {
      ...
      if (this._siblingMode) {
        PFireEvent('pedigree:person:newsibling', event);
      } else {
        PFireEvent('pedigree:person:newpartnerandchild', event);
      }
      ...
      PFireEvent('pedigree:partnership:newchild', event);
    }
    this.hide();
  },
  ...
  generateExpandArrow: function(data) {
    ...
    expandArrow.expand = function() {
      ...
      expandArrow._p_update('▴');
      expandArrow._p_removeClassName('collapsed');
    }.bind(this);

    expandArrow.collapse = function() {
      this.expandedOptionsContainer._p_update('');
      expandArrow._p_update('▾');
      expandArrow._p_addClassName('collapsed');
    }.bind(this);

    expandArrow._p_observe('click', function() {
      console.log('observe2');
      if(expandArrow._p_hasClassName('collapsed')) {
        expandArrow.expand();
      } else {
  ...
  _generateSeparator : function() {
    return PElement('span', {'class' : 'separator'})._p_update(' | ');
  },
  ...
  _positionAt : function(x, y) {
    y = Math.round(y);
    if (y + this.element._p_getHeight() > editor.getWorkspace().getWorkArea()._p_getHeight()) {
      this.element._p_addClassName('upside');
      y = Math.round(y - this.element._p_getHeight());
    }
    this.element.style.top = y + 'px';
    var dx = Math.round(this.element._p_getWidth()/2);
    if (x - dx + this.element._p_getWidth() > editor.getWorkspace().getWorkArea()._p_getWidth()) {
      dx = Math.round(this.element._p_getWidth() - (editor.getWorkspace().getWorkArea()._p_getWidth() - x));
    } else if (dx > x) {
      dx = Math.round(x);
    }
    this.element._p_down('.callout-handle').style.left = dx + 'px';
  ...
  show : function(node, x, y) {
    ...
    //   this.element.querySelectorAll('.marker-type-option')._p_invoke('hide');
    // } else if (/* the node cannot have children */) {
    //   this.element.querySelectorAll('.person-type-option')._p_invoke('hide');
    // }
    this.element._p_show();
    this.expandedOptionsContainer._p_update('');
  ...
  hide : function() {
    ...
    if (this._node) {
      ...
      delete this._node;
      // reset the state
      //$A(this.element.querySelectorAll('.node-type-option'))._p_invoke('show');
      this.element._p_removeClassName('upside');
    }
    this.element._p_hide();
  ...
  _onClickOutside: function (event) {
    ...
    if (!getEventMatchingParentElement(event, '.callout')) {
      this.hide();
    }
  },
  ...
  expandPersonGroup: function(personGroupMenuInfo) {
    ...
    var createBtn = PElement('input', {'type': 'button', 'value': 'create', 'class': 'button'});
    var svgContainer = PElement('span')._p_update(generateIcon());
    var minusBtn = PElement('span', {
      'class': 'minus-button value-control-button'
    })._p_update('-');
    var plusBtn = PElement('span', {
    ...
    this.expandedOptionsContainer._p_insert(minusBtn);
    this.expandedOptionsContainer._p_insert(svgContainer);
    this.expandedOptionsContainer._p_insert(plusBtn);
    this.expandedOptionsContainer._p_insert(createBtn);

    //        var height = this.element._p_getHeight();
  ...
  expandTwins: function(twinMenuInfo) {
    ...
    minusBtn._p_observe('click', function() {
      me._decrementNumTwins(); svgContainer._p_update(generateIcon());
    });
    plusBtn._p_observe('click',  function() {
      me._incrementNumTwins(); svgContainer._p_update(generateIcon());
    });
    createBtn._p_observe('click', function() {
