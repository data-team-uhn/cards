...
import { Timer, removeFirstOccurrenceByValue } from '../model/helpers';
...
var Person = Class.create(AbstractPerson, {
  ...
  removeDisorder: function(disorderID) {
    if(this.hasDisorder(disorderID)) {
      ...
      removeFirstOccurrenceByValue(this._disorders, disorderID);
    } else {
  ...
  removeHPO: function(hpoID) {
    if(this.hasHPO(hpoID)) {
      ...
      removeFirstOccurrenceByValue(this._hpo, hpoID);
    } else {
  ...
  removeGene: function(gene) {
    if (this.getGenes().indexOf(gene) !== -1) {
      ...
      removeFirstOccurrenceByValue(this._candidateGenes, gene);
    }
  },
...
Person._p_addMethods(ChildlessBehavior);
