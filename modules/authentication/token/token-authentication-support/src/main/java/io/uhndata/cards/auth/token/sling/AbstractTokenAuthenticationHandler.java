 ...
 * under the License.
 */
package io.uhndata.cards.auth.token.sling;
...
/**
 * Base class for token-based authentication handlers, providing shared logic for reading tokens from requests and
 * cookies, setting the authentication cookie on success, and redirecting to the expiry page on failure. Subclasses must
 * provide a {@link TokenManager} instance and a token format validator.
 *
 * @version $Id$
 */
public abstract class AbstractTokenAuthenticationHandler extends DefaultJakartaAuthenticationFeedbackHandler
    implements JakartaAuthenticationHandler
{
    /** The name of the request parameter that may hold a login token. */
    protected static final String TOKEN_REQUEST_PARAMETER = "auth_token";

    /** The name of the cookie that may hold a login token. */
    protected static final String TOKEN_COOKIE_NAME = "cards_auth_token";

    /**
     * Returns the {@link TokenManager} to use for parsing and creating tokens.
     *
     * @return the token manager, never {@code null} during normal operation
     */
    protected abstract TokenManager getTokenManager();

    /**
     * Returns {@code true} if the given string matches the token format handled by this handler.
     *
     * @param token the candidate token string, may be {@code null}
     * @return {@code true} if the string is a token this handler should be able to process
     */
    protected abstract boolean isValidTokenString(String token);
    ...
    /**
     * Given a login token, determine its expiration date.
     *
    ...
    {
        if (!isValidTokenString(loginToken)) {
        ...
        // Try to parse it
        final TokenInfo token = getTokenManager().parse(loginToken);
