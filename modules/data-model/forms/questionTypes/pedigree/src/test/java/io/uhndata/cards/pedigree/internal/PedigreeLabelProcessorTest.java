 ...
 * under the License.
 */
package io.uhndata.cards.pedigree.internal;
...
/**
 * Unit tests for {@link PedigreeLabelProcessor}.
 *
 * @version $Id $
 */
@SuppressWarnings("unchecked")
@RunWith(MockitoJUnitRunner.class)
public class PedigreeLabelProcessorTest
{
    ...
    private static final String ANSWER_PEDIGREE_TYPE = "cards:PedigreeAnswer";
    ...
    @InjectMocks
    private PedigreeLabelProcessor pedigreeLabelProcessor;

    @Test
    public void getNameTest()
    {
        Assert.assertEquals(NAME, this.pedigreeLabelProcessor.getName());
    }

    @Test
    public void getDescriptionReturnsSomething()
    {
        Assert.assertNotNull(this.pedigreeLabelProcessor.getDescription());
    }

    @Test
    public void getPriorityTest()
    {
        Assert.assertEquals(PRIORITY, this.pedigreeLabelProcessor.getPriority());
    }

    @Test
    public void isEnabledByDefaultReturnsTrue()
    {
        Assert.assertEquals(ENABLED, this.pedigreeLabelProcessor.isEnabledByDefault(mock(Resource.class)));
    }
    ...
    public void canProcessForFormReturnsTrue()
    {
        ...
        Assert.assertTrue(this.pedigreeLabelProcessor.canProcess(form));
    }
    ...
    public void canProcessForQuestionnaireReturnsFalse()
    {
        ...
        Assert.assertFalse(this.pedigreeLabelProcessor.canProcess(questionnaire));
    }

    @Test
    public void leaveForPedigreeAnswerNode() throws RepositoryException
    {
        Session session = this.context.resourceResolver().adaptTo(Session.class);
        JsonObjectBuilder json = Json.createObjectBuilder();
        Node node = session.getNode("/Forms/f1/a1");
        String imageSvg = "<svg></svg>";
        node.setProperty(VALUE_PROPERTY, imageSvg);
        node.setProperty(IMAGE_PROPERTY, imageSvg);

        this.pedigreeLabelProcessor.leave(node, json, mock(Function.class));
        ...
        Assert.assertEquals(imageSvg, jsonObject.getString(DISPLAYED_VALUE_PROPERTY));
    }

    @Test
    public void leaveForPedigreeAnswerNodeWithImagePropertyThrowsException() throws RepositoryException
    {
        JsonObjectBuilder json = Json.createObjectBuilder();
        Node node = mock(Node.class);
        when(node.isNodeType(ANSWER_PEDIGREE_TYPE)).thenReturn(true);
        when(node.hasProperty(VALUE_PROPERTY)).thenReturn(true);
        when(node.hasProperty(QUESTION_PROPERTY)).thenReturn(false);
        when(node.getProperty(IMAGE_PROPERTY)).thenThrow(new RepositoryException());
    ...
    @Test
    public void leaveForNotPedigreeAnswerNodeThrowsException() throws RepositoryException
    {
        JsonObjectBuilder json = Json.createObjectBuilder();
        Node node = mock(Node.class);
        when(node.isNodeType(ANSWER_PEDIGREE_TYPE)).thenThrow(new RepositoryException());
    ...
    public void setupRepo() throws RepositoryException
    {
        ...
        try (Reader cnd = new InputStreamReader(
            getClass().getResourceAsStream("/SLING-INF/nodetypes/pedigree.cnd"), StandardCharsets.UTF_8)) {
        ...
        this.context.build()
            ...
            .resource("/Forms/f1/a1",
                NODE_TYPE, ANSWER_PEDIGREE_TYPE,
