...
function NewFormDialog(props) {
  const { classes, presetPath, currentSubject, theme, open, onClose, withButton, buttonTitle, extensionURL } = {open: false, ...props };
    ...
    fetchWithReLogin(globalLoginDisplay, URL, { method: 'POST', body: request_data })
      .then( (response) => {
        ...
        if (response.ok) {
          ...
          props.history.push("/content.html" + (extensionURL ? "/" + extensionURL : "") + URL + '.edit');
        } else {
