...
function ResourceQuestion(props) {
  ...
  useEffect(() => {
    const url = (
      primaryType
        ? `/query?query=select * from [${primaryType}] as n order by n.'cards:defaultOrder',`
        + ` n.'${labelProperty}'&limit=${MAX_TO_DISPLAY}`
        : undefined
    );
...
// Contribute the "resource" dataType to the question editor, in place of a hardcoded entry in Question.json.
