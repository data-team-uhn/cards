...
const reorderReducer = (state, action) => {
    ...
    switch (action.type) {
        ...
        case 'SET_NEWPOSITION':
            newInputs = {
                ...state.inputs, newPosition: action.payload
            }
            return { ...state, inputs: newInputs }
        default:
...
export default function ReorderForm(props) {
    ...
    const [reorderState, reorderDispatch] = useReducer(reorderReducer, disableReorderSourceSelect ? { ...initialReorderState, inputs: { ...initialReorderState.inputs, reorderSource: data['jcr:uuid'] } } : initialReorderState)
    ...
    useEffect(() => {
        if (Object.keys(nodes).length === 0) {
            ...
            if (!reorderSource.length) {
                ...
                console.log('Setting original parent and position')
                setNewParentSelection([originalParent])
                setNewPositionSelection([originalPosition])
            }
        } else {
            console.log(reorderSource, reorderState)
            const originalParent = nodes[reorderSource].parent
            const originalPosition = nodes[originalParent].children.indexOf(reorderSource)
            console.log('Setting original parent and position')
            setNewParentSelection([originalParent])
    ...
    const SelectReorderSourceAutocomplete = (props) => {
        return (
            <>
                ...
                <Grid item xs={disableReorderSourceSelect ? 12 : 10}>
                    {
            ...
            <Grid container alignItems='baseline' direction="row" rowSpacing={3} columnSpacing={2}>
                ...
                {!!reorderSource.length && (
                    <>
                        ...
                        <Grid item xs={10}>
                            <Typography>
                                {getOrdinalString(nodes[nodes[reorderSource].parent].children.indexOf(reorderSource))}
                            </Typography>
                        </Grid>
                    </>
                )}
    ...
    const handleSubmit = (e) => {
        ...
        console.log('reorder', reorderSource, newParent, newPosition, reorderState)
