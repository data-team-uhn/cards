...
import { useQuestionnaireTreeContext, ENTRY_TITLE_FIELD_SPEC, jcrGetConditionalTitle, jcrActions } from './QuestionnaireTreeContext';
...
const useReorderSourceStyles = makeStyles()((theme, props) => {
    ...
    return ({
        selectedReorderSource: !type ? {} : {
            // border: `2px solid ${ENTRY_TITLE_FIELD_SPEC[`cards:${props.type}`]?.color}`,
...
const reorderReducer = (state, action) => {
    ...
    switch (action.type) {
        ...
        case 'SET_TARGET_AND_MOVE':
            ...
            // window.alert(`Reordering source ${reorderSourceId} to target ${reorderTargetId} at index ${reorderTargetIndex} in parent ${targetParent}`)
    ...
    const onConfirm = async () => {
        ...
        const processMoves = async (moves) => {
            for (let i = 0; i < moves.length; i++) {
                ...
                // console.log('reorder promise', reorderSourceId, reorderNewParentId, reorderTargetIndex)
                try {
                    const { reorderSourceId, reorderNewParentId, reorderTargetIndex } = move
...
const TargetPlaceholderDivider = (props) => {
    ...
    if (!sourceIsSelected) {
        ...
        const nextNodeType = parentChildren[index + 1] ? reorderState.draftTree[parentChildren[index + 1]].jcrPrimaryType : null


        let leftBorderColor = ENTRY_TITLE_FIELD_SPEC[reorderState.draftTree[nodeId].jcrPrimaryType]?.color
        ...
        return (
            <div style={{borderLeft: `3px solid ${leftBorderColor}`, height: '16px'}} />
        )
    }
    ...
    if (isInvalid) {
        if (isInvalidReason === INVALID_REASONS['EXISTING_CHILD']) {
            return (
                ...
                >
                    <div 
                        className={classes.invalidTargetPlaceholderItem}
                    >
                    </div>
                </Tooltip>
            )
...
function RecursiveDragList(props) {
    ...
    if (!nodes) {
        console.log('Tree not loaded yet, rendering null recursive list')
        return null
    }
    ...
    // console.log('highlighted items', inView.highlighter.highlightedItems)
    ...
    return (
        <>
            {!isRootNode && (
                <TargetPlaceholderDivider nodeId={nodeId} level={level} />
            )}
            ...
            >
                {
                    ...
                    !!conditionalChildren.length && (
                        <ReorderConditionalSubheader node={node} />
                    )
                }
                {
                    !isRootNode && (
                                ...
                                <ListItemSecondaryAction>
                                    {!nodeIsSource && (
                                        <Tooltip title={'Select to move'}>
                                            <IconButton
                                                onClick={() => { handleClickReorderSourceSelect() }}
                                            >
                ...
                {collapsible && !collapsed && (
                    <Collapse in={!collapsed} timeout="auto" unmountOnExit>
                        ...
                        >
                            {entryChildren.map((childId) => (
                                <RecursiveDragList key={childId} nodeId={childId} level={level + 1} />
                            ))}
    ...
    return (
        <>
            <ReorderProvider tree={treeContext.state.nodes}>
                ...
                <List dense
                    className={`${reorderStyles.classes.selectionList}`}
                >
