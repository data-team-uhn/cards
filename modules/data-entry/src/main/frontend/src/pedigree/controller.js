import { Class, $, PObserveEvent } from './shims/prototypeShim';

import { cloneObject, Timer } from './model/helpers';
...
var Controller = Class.create({
  initialize: function() {
    PObserveEvent('pedigree:graph:clear',               this.handleClearGraph);
    PObserveEvent('pedigree:undo',                      this.handleUndo);
    PObserveEvent('pedigree:redo',                      this.handleRedo);
    PObserveEvent("pedigree:historychange",             this.handleUndoHistoryChange);
    PObserveEvent("pedigree:autolayout",                this.handleAutoLayout);
    PObserveEvent('pedigree:node:remove',               this.handleRemove);
    PObserveEvent('pedigree:node:setproperty',          this.handleSetProperty);
    PObserveEvent('pedigree:node:modify',               this.handleModification);
    PObserveEvent('pedigree:person:drag:newparent',     this.handlePersonDragToNewParent);
    PObserveEvent('pedigree:person:drag:newpartner',    this.handlePersonDragToNewPartner);
    PObserveEvent('pedigree:person:drag:newsibling',    this.handlePersonDragToNewSibling);
    PObserveEvent('pedigree:person:newparent',          this.handlePersonNewParents);
    PObserveEvent('pedigree:person:newsibling',         this.handlePersonNewSibling);
    PObserveEvent('pedigree:person:newpartnerandchild', this.handlePersonNewPartnerAndChild);
    PObserveEvent('pedigree:partnership:newchild',      this.handleRelationshipNewChild);
  },
  ...
  handleUndoHistoryChange: function () {
    ...
    if (editor.getActionStack().hasRedo()) {
      redoButton._p_removeClassName("disabled-menu-item");
      redoButton._p_addClassName("menu-item");
    } else {
      redoButton._p_removeClassName("menu-item");
      redoButton._p_addClassName("disabled-menu-item");
    }
    var undoButton = $('action-undo');
    if (editor.getActionStack().hasUndo()) {
      undoButton._p_removeClassName("disabled-menu-item");
      undoButton._p_addClassName("menu-item");
    } else {
      undoButton._p_removeClassName("menu-item");
      undoButton._p_addClassName("disabled-menu-item");
    }
  },
