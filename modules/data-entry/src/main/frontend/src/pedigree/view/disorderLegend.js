...
import { isInt, removeFirstOccurrenceByValue } from '../model/helpers';
...
var DisorgerLegend = Class.create( Legend, {
  ...
  _updateDisorderName: function(disorderID) {
    var name = this._legendBox._p_down('li#' + this._getPrefix() + '-' + disorderID + ' .disorder-name');
    name._p_update(this.getDisorder(disorderID).getName());
  },
  ...
  _generateElement: function ($super, disorderID, name) {
    if (!this._objectColors.hasOwnProperty(disorderID)) {
      ...
      PFireEvent('disorder:color', {'id' : disorderID, color: color});
    }
