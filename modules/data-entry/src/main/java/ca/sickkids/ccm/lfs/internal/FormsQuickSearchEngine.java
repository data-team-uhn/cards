...
/**
 * Finds {@code [lfs:Forms]}s with answers or notes matching the given full text search.
 *
 * @version $Id$
 */
...
{
    private static final List<String> SUPPORTED_TYPES = Collections.singletonList("lfs:Form");
    ...
    /**
     * Get the ancestor {@code lfs:Form} node that a matched answer node belongs to.
     *
     * @param answer an answer node matched by the query
    ...
    private Resource getForm(Resource answer)
    {
        ...
        while (result != null && !"lfs/Form".equals(result.getResourceType())) {
