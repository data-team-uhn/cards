...
public class AnthropicServlet extends SlingJakartaAllMethodsServlet
{
    ...
    private List<AnthropicMessage> toMessageList(final JsonArray messages)
    {
        final List<AnthropicMessage> result = new ArrayList<>(messages.size());
        for (final JsonObject msg : messages.getValuesAs(JsonObject.class)) {
            result.add(new AnthropicMessage(msg.getString("role"), msg.getString("content")));
        }
        return result;
    }
