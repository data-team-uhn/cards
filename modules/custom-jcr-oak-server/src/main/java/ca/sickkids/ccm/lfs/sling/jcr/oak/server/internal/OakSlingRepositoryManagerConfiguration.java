...
@ObjectClassDefinition(
    ...
    description = "Configuration to launch an embedded JCR Repository and provide it as a SlingRepository and a "
        + "standard JCR Repository. In addition, if the registration URL is not empty, the repository is registered "
        + "as defined.")
@interface OakSlingRepositoryManagerConfiguration
{
    ...
    @AttributeDefinition(
        ...
        description = "Name of the workspace to use by default if not is given in any of the login methods. "
            + "This name is used to implement the SlingRepository.getDefaultWorkspace() "
    ...
    @AttributeDefinition(
        ...
        description = "Use the Whiteboard dynamically for looking up RepositoryInitializer-s, CommitHook-s, and "
            + "EditorProvider-s. This is potentially dangerous, since it allows bundles to access all data changes.")
