 ...
 * The parsing and node creation process is done as a transaction, meaning that if it fails, then proposed changes
 * saved in storage will not be applied, and the repository will be left in its original state.
 * <p>
 * The parser assumes that the resource of the response it is given is a <code>VocabulariesHomepage</code> node
 * under which the <code>Vocabulary</code> node instance should be stored in the Jackrabbit Oak repository as a child.
 * The homepage node is obtained by adapting the resource of the response from a
...
public abstract class AbstractNCITParser implements VocabularyParser
{
     ...
     * </p>
     * The method obtains the <code>VocabulariesHomepage</code> node by getting the resource of the request and
     * adapting it from a {@link org.apache.sling.api.resource.Resource} to a {@link javax.jcr.node}.
            ...
            throws IOException
    {
        ...
        try {
             ...
             *  If any errors occur before this step, all proposed changes will not be applied and the repository
             *  will remain in its original state.
             *  Lucene indexing is automatically performed by the Jackrabbit Oak repository when this is performed.
             */
    ...
    /**
     * Creates a <code>Vocabulary</code> node that represents the current vocabulary instance with the identifier.
     * as the name of the node. The vocabulary property <code>website</code> is currently fixed to
    ...
    /**
     * Creates a <code>VocabularyTerm</code> node representing an individual term of the NCIT.
     * This method is protected to allow subclass implementations of {@link parseNCIT} to use this method, allowing
     * the node creation process to be standardized.
     * <p>
     ...
     * successful, then the changes made already will be applied to the JCR repository. If not, then all of the
     * changes will not be applied. After the session is saved, then the JCR repository will automatically begin
     * Lucene indexing.
