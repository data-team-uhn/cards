...
/**
 * A {@link RangeMinMaxAnswersValidator} reports if a given number of answers is invalid for a given question, when the
 * question is marked as a range question.
 *
 * @version $Id$
 */
@Component(immediate = true)
public class RangeMinMaxAnswersValidator extends MinMaxAnswersValidator implements AnswerValidator
{
    @Override
        ...
        final Map<String, Boolean> flags)
    {
        try {
            ...
            final boolean isRange =
                question.hasProperty("isRange") ? question.getProperty("isRange").getBoolean() : false;
