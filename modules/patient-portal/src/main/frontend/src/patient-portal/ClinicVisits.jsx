    ...
    },
    {
      ...
      "format" : (row) => (
         <Link
           ...
           underline="hover">
           { row.email_unsubscribed || row?.unsubscribed_list.includes(`/Survey/ClinicMapping/${clinicId}`) ? "Unsubscribed" : (!row.has_surveys ? "No surveys assigned" : (!row.surveys_complete ? "Incomplete" : (!row.surveys_submitted ? "Pending submission" : "Completed"))) }
         </Link>
      )
    }
  ];
