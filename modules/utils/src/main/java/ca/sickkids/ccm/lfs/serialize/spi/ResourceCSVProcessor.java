 ...
 * {@link ca.sickkids.ccm.lfs.serialize.ResourceToCSVAdapterFactory} when serializing a resource as {@code CSVString}.
 * When serializing a resource, each enabled processors will be asked if they {@link #canProcess(Resource) can process}
 * the resource, and the first one to answer {@code true} will have it's {@link #serialize(Resource)} method invoked
 * with the resource as a parameter.
 *
 * @version $Id$
 */
