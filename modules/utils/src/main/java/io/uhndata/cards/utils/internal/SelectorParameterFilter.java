 ...
 * path. For security, the request path cannot contain special characters like slash and backslash, and while dots are
 * allowed, in order to be used inside a selector instead of being interpreted as a separator between selectors they
 * would need to be escaped. A query parameter does not have any constraints, but Sling's own selector parsing only
 * works on the path. This filter records the selectors passed as query parameters for the duration of the request into
 * the {@link io.uhndata.cards.utils.SelectorUtils} helper.
 *
 * @version $Id$
 */
