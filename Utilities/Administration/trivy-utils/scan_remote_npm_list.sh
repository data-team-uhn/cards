...
docker run --rm -v $(realpath $PACKAGE_CONF_DIR)/$DEPLOYMENT_HOSTNAME/docker/cards/yarn.lock:/yarn.lock aquasec/trivy fs --security-checks vuln --ignore-unfixed /yarn.lock --format json | python3 trivy_to_slack.py > $SLACK_MESSAGES_FILE
