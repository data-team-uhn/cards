...
#
# Because this module both aggregates the features and builds the image, the preparation runs
# in two steps: `harvest` right after the aggregates are built (prepare-package), and
# `mvnrepo` after the current build's artifacts were deployed to .mvnrepo (verify).
...
then
  ...
  mkdir -p "$CONTEXT/artifacts" "$CONTEXT/metadata" "$TARGET/generated-features"

  # Harvest every feature file built by the current reactor. The processed (fully
  # substituted) feature of each module is left by the slingfeature-maven-plugin in
  # target/slingfeature-tmp/. Each harvested copy gets its id rewritten to this module's
  # coordinates, with a classifier derived from the original feature id, since that is how
  # the `repository` goal expects mid-build features to identify themselves. Only modules
  # tracked by git are harvested, so that leftover target directories of modules from other
  # branches are not picked up.
  find "$ROOT_DIR" -path '*/target/slingfeature-tmp/*.json' \
    ...
    | python3 "$MODULE_DIR/harvest_features.py" "io.uhndata.cards:cards:$VERSION" \
...
EOF
  ...
  cp "$TARGET/slingfeature-tmp/feature-core_tar.json" "$CONTEXT/metadata/core_tar.json"
  cp "$TARGET/slingfeature-tmp/feature-core_mongo.json" "$CONTEXT/metadata/core_mongo.json"
...
then
  # The project artifact repository, used by the feature launcher to resolve the feature
  # files themselves and every project-built bundle. Copied only after the current build's
  # artifacts, including this module's aggregated features, were deployed to it.
  rm -rf "$CONTEXT/mvnrepo"
  mkdir -p "$CONTEXT/mvnrepo"
  cp -a "$ROOT_DIR/.mvnrepo/." "$CONTEXT/mvnrepo/"
  echo "Project artifact repository copied into $CONTEXT/mvnrepo"
else
  echo "Usage: prepare_context.sh harvest|mvnrepo <version>" >&2
  exit 1
fi
